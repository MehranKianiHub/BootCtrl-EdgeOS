...
namespace forte::eclipse4diac::edgeml {
    ...
    class InferenceRuntime {
      public:
        InferenceRuntime() {
          ...
          const auto status = mBackend.loadModel(metadata, binary);
          mReady = EEdgeMLError::kOk == status || EEdgeMLError::kModelAlreadyExists == status;
        }
  ...
  void FORTE_ML_Inference::executeEvent(const TEventID paEIID, CEventChainExecutionThread *const paECET) {
    ...
    const auto status = inferenceRuntime.backend().infer(var_MODEL_ID.getStorage(), input, output, stats);
