...
#include "forte/eclipse4diac/edgeml/postprocessing/ML_Softmax_fbt.h"
...
namespace forte::eclipse4diac::edgeml {
  ...
  DEFINE_FIRMWARE_FB(FORTE_ML_Softmax, "eclipse4diac::edgeml::ML_Softmax"_STRID)

  FORTE_ML_Softmax::FORTE_ML_Softmax(const StringId paInstanceNameId, CFBContainer &paContainer) :
  ...
  void FORTE_ML_Softmax::setInitialValues() {
    var_IN_0 = CIEC_REAL(0.0F);
    var_IN_1 = CIEC_REAL(0.0F);
    var_IN_2 = CIEC_REAL(0.0F);
    var_IN_3 = CIEC_REAL(0.0F);
    var_TEMPERATURE = CIEC_REAL(1.0F);

    var_OUT_0 = CIEC_REAL(0.25F);
    var_OUT_1 = CIEC_REAL(0.25F);
    var_OUT_2 = CIEC_REAL(0.25F);
    var_OUT_3 = CIEC_REAL(0.25F);
    var_VALID = CIEC_BOOL(true);
    var_ERROR = CIEC_BOOL(false);
    var_ERROR_CODE = CIEC_USINT(scmErrorOk);
  }

  void FORTE_ML_Softmax::setError(const CIEC_USINT::TValueType paErrorCode) {
  ...
  void FORTE_ML_Softmax::clearError() {
    var_VALID = CIEC_BOOL(true);
    var_ERROR = CIEC_BOOL(false);
    var_ERROR_CODE = CIEC_USINT(scmErrorOk);
  }

  void FORTE_ML_Softmax::executeEvent(const TEventID paEIID, CEventChainExecutionThread *const paECET) {
    if (scmEventREQID != paEIID) {
      return;
    }

    if (!isFinite(var_IN_0) || !isFinite(var_IN_1) || !isFinite(var_IN_2) || !isFinite(var_IN_3) ||
        !isFinite(var_TEMPERATURE)) {
      setError(scmErrorNonFiniteInput);
      sendOutputEvent(scmEventCNFID, paECET);
      return;
    }

    const auto temperature = static_cast<CIEC_REAL::TValueType>(var_TEMPERATURE);
    if (!(temperature > 0.0F)) {
      setError(scmErrorInvalidTemperature);
      sendOutputEvent(scmEventCNFID, paECET);
      return;
    }

    std::array<CIEC_REAL::TValueType, 4> logits{static_cast<CIEC_REAL::TValueType>(var_IN_0),
    ...
    const auto maxLogit = *std::max_element(logits.begin(), logits.end());

    std::array<CIEC_REAL::TValueType, 4> exponentials{};
    CIEC_REAL::TValueType sumExp = 0.0F;
    for (size_t i = 0; i < logits.size(); ++i) {
      exponentials[i] = std::exp(logits[i] - maxLogit);
      if (!std::isfinite(exponentials[i])) {
        setError(scmErrorNumericalFailure);
        sendOutputEvent(scmEventCNFID, paECET);
        return;
      }
    ...
    const auto out0 = exponentials[0] / sumExp;
    const auto out1 = exponentials[1] / sumExp;
    const auto out2 = exponentials[2] / sumExp;
    const auto out3 = exponentials[3] / sumExp;
    if (!std::isfinite(out0) || !std::isfinite(out1) || !std::isfinite(out2) || !std::isfinite(out3)) {
      setError(scmErrorNumericalFailure);
      sendOutputEvent(scmEventCNFID, paECET);
      return;
    }

    var_OUT_0 = CIEC_REAL(out0);
    var_OUT_1 = CIEC_REAL(out1);
    var_OUT_2 = CIEC_REAL(out2);
    var_OUT_3 = CIEC_REAL(out3);
    clearError();

    sendOutputEvent(scmEventCNFID, paECET);
  }

  void FORTE_ML_Softmax::readInputData(const TEventID paEIID) {
    switch (paEIID) {
      case scmEventREQID: {
        ...
        readData(4, var_TEMPERATURE, conn_TEMPERATURE);
        break;
      }
      default: break;
    }
  ...
  void FORTE_ML_Softmax::writeOutputData(const TEventID paEIID) {
    switch (paEIID) {
      case scmEventCNFID: {
  ...
  CIEC_ANY *FORTE_ML_Softmax::getDI(const size_t paIndex) {
    switch (paIndex) {
      case 0: return &var_IN_0;
      case 1: return &var_IN_1;
      case 2: return &var_IN_2;
      case 3: return &var_IN_3;
      case 4: return &var_TEMPERATURE;
    }
    return nullptr;
  }

  CIEC_ANY *FORTE_ML_Softmax::getDO(const size_t paIndex) {
    switch (paIndex) {
  ...
  CEventConnection *FORTE_ML_Softmax::getEOConUnchecked(const TPortId paIndex) {
    switch (paIndex) {
  ...
  CDataConnection **FORTE_ML_Softmax::getDIConUnchecked(const TPortId paIndex) {
    switch (paIndex) {
      case 0: return &conn_IN_0;
      case 1: return &conn_IN_1;
      case 2: return &conn_IN_2;
      case 3: return &conn_IN_3;
      case 4: return &conn_TEMPERATURE;
    }
    return nullptr;
  }

  CDataConnection *FORTE_ML_Softmax::getDOConUnchecked(const TPortId paIndex) {
    switch (paIndex) {
