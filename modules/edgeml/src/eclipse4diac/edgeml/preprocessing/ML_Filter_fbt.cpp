...
#include "forte/eclipse4diac/edgeml/preprocessing/ML_Filter_fbt.h"
...
namespace forte::eclipse4diac::edgeml {
  namespace {
    const auto cDataInputNames =
        std::array{"IN_0"_STRID, "IN_1"_STRID, "IN_2"_STRID, "IN_3"_STRID, "ALPHA"_STRID, "RESET"_STRID};
  ...
  DEFINE_FIRMWARE_FB(FORTE_ML_Filter, "eclipse4diac::edgeml::ML_Filter"_STRID)

  FORTE_ML_Filter::FORTE_ML_Filter(const StringId paInstanceNameId, CFBContainer &paContainer) :
      ...
      mPrevOutput{0.0F, 0.0F, 0.0F, 0.0F},
      mInitialized(false) {
    setInitialValues();
  }

  void FORTE_ML_Filter::setInitialValues() {
    var_IN_0 = CIEC_REAL(0.0F);
    var_IN_1 = CIEC_REAL(0.0F);
    var_IN_2 = CIEC_REAL(0.0F);
    var_IN_3 = CIEC_REAL(0.0F);
    var_ALPHA = CIEC_REAL(0.5F);
  ...
  void FORTE_ML_Filter::setError(const CIEC_USINT::TValueType paErrorCode) {
    var_OUT_0 = CIEC_REAL(0.0F);
    var_OUT_1 = CIEC_REAL(0.0F);
    var_OUT_2 = CIEC_REAL(0.0F);
    var_OUT_3 = CIEC_REAL(0.0F);
  ...
  void FORTE_ML_Filter::clearError() {
    var_VALID = CIEC_BOOL(true);
    var_ERROR = CIEC_BOOL(false);
    var_ERROR_CODE = CIEC_USINT(scmErrorOk);
  }

  void FORTE_ML_Filter::executeEvent(const TEventID paEIID, CEventChainExecutionThread *const paECET) {
    ...
    const auto alpha = static_cast<CIEC_REAL::TValueType>(var_ALPHA);
    if (alpha < 0.0F || alpha > 1.0F) {
      setError(scmErrorInvalidAlpha);
      sendOutputEvent(scmEventCNFID, paECET);
      return;
    }

    const std::array<CIEC_REAL::TValueType, scmVectorWidth> input{static_cast<CIEC_REAL::TValueType>(var_IN_0),
    ...
    std::array<CIEC_REAL::TValueType, scmVectorWidth> output{};
    if (!mInitialized) {
      output = input;
      mInitialized = true;
    } else {
      for (std::size_t i = 0; i < scmVectorWidth; ++i) {
        output[i] = alpha * input[i] + (1.0F - alpha) * mPrevOutput[i];
      }
    }
  ...
  void FORTE_ML_Filter::readInputData(const TEventID paEIID) {
    switch (paEIID) {
      case scmEventREQID: {
        ...
        readData(4, var_ALPHA, conn_ALPHA);
  ...
  void FORTE_ML_Filter::writeOutputData(const TEventID paEIID) {
    switch (paEIID) {
      case scmEventCNFID: {
  ...
  CIEC_ANY *FORTE_ML_Filter::getDI(const size_t paIndex) {
    switch (paIndex) {
      case 0: return &var_IN_0;
      case 1: return &var_IN_1;
      case 2: return &var_IN_2;
      case 3: return &var_IN_3;
      case 4: return &var_ALPHA;
      case 5: return &var_RESET;
    }
    return nullptr;
  }

  CIEC_ANY *FORTE_ML_Filter::getDO(const size_t paIndex) {
    switch (paIndex) {
  ...
  CEventConnection *FORTE_ML_Filter::getEOConUnchecked(const TPortId paIndex) {
    switch (paIndex) {
  ...
  CDataConnection **FORTE_ML_Filter::getDIConUnchecked(const TPortId paIndex) {
    switch (paIndex) {
      case 0: return &conn_IN_0;
      case 1: return &conn_IN_1;
      case 2: return &conn_IN_2;
      case 3: return &conn_IN_3;
      case 4: return &conn_ALPHA;
      case 5: return &conn_RESET;
    }
    return nullptr;
  }

  CDataConnection *FORTE_ML_Filter::getDOConUnchecked(const TPortId paIndex) {
    switch (paIndex) {
